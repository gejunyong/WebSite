		...
		<div class="cw">
			...
			<div class="clearfix main">
				...
				<div class="content fr pt20">
					...
					<ul class="proList mt30 clearfix">
					<c:forEach items="${ps }" var="p">
					<li style="background-image:url('<%=request.getContextPath() %>/${p.imgPath}')">
					<h3>
					<c:if test="${p.pId eq 1 }">
					<a href="<%=request.getContextPath() %>/index/products/categorys/${p.id }">${p.name}</a>
					</c:if>
					<c:if test="${p.pId ne 1 }">
					<a href="<%=request.getContextPath() %>/index/products/products/${p.id }">${p.name}</a>
					</c:if>
					</h3></li>               
					</c:forEach>
					</ul>
